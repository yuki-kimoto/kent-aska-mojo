...
get '/' => sub {
  ...
  if ($method eq 'post') {
    if ($mode eq 'regist') {
      ...
      my $errors = [];
      if ($config->{no_wd}) {
        if (my $flg = no_wd("$name $sub $comment")) {
          push @$errors, ["禁止ワードが含まれています"];
        }
      }
      ...
      if ($config->{use_captcha} > 0) {
        ...
        #  0 : 制限時間オーバー
        #  1 : キー一致
        my $chk = cap::check($captcha,$in{str_crypt},$config->{captcha_key},$config->{cap_time},$config->{cap_len});
        if ($chk == 0) {
      ...
      # フォーム内容をチェック
      my $err;
      if ($name eq "") { $err .= "名前が入力されていません<br />"; }
      if ($comment eq "") { $err .= "コメントが入力されていません<br />"; }
      ...
      # 更新
      seek(DAT, 0, 0);
      print DAT "$no<>$date<>$name<>$in{email}<>$sub<>$comment<>$in{url}<>$host<>$pwd<>$time<>\n";
      print DAT @data;
      truncate(DAT, tell(DAT));
      close(DAT);

      # クッキー格納
      set_cookie($name,$in{email},$in{url}) if ($in{cookie} == 1);
...
EOS
    ...
    # 修正実行
      ...
      while (<DAT>) {
        ...
        if ($in{no} == $no) {
          $_ = "$no<>$dat<>$name<>$in{email}<>$sub<>$comment<>$in{url}<>$hos<>$pwd<>$tim<>\n";
        }
        push(@data,$_);
      }
