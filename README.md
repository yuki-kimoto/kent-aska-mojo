...
## ダウンロード(開発版。まだ正しく動きません)

以下の利用方法で、ダウンロードするディレクトリが変わります。

###A. suEXEC環境を提供している共有レンタルサーバーにCGIとして設置する場合

これはさくらのレンタルサーバー・スタンダードなどが該当します。
CGIを設置するディレクトリに移動してください。
この場所は、サービスによって異なります。

    cd ~/www

### B. 自己管理のサーバー上にCGIとして設置する場合

まずroot権限になります。

    su -

次に、Webサーバーのドキュメントルートに移動します。
この場所は、Webサーバーの設定によって異なります。

    cd /var/www/html

### C.自己管理のサーバー上で、組み込みWebサーバーを使って起動する場合
...
-WebフレームワークとしてMojoliciousを利用。フォームのパースなどのHTTPの処理ロジックをMojoliciousにまかせた。
-Mojo::Templateを使って、テンプレートの記述を改善。
-必要モジュールのインストールのシステムとしてcpanmを使用
-Emailの送信にMIME::Liteを使用
